document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateMultiplicacion(cA) {
        const minA = Math.pow(10, cA - 1);
        const maxA = Math.pow(10, cA) - 1;

        // Generar multiplicando de N cifras y multiplicador de 1 cifra (del 2 al 9)
    ...
    function renderMultiplicacion(data) {
        ...
        // Los pasos del juego van de DERECHA a IQUIERDA (última cifra en adelante)
        gameSteps = [...resultCells].reverse();
    }

    function initGame() {
        const data = generateMultiplicacion(cifrasA);
    ...
    function handleInput(val) {
        ...
        if (val === 'backspace') {
            if (currentStepIndex > 0) {
                // Si la celda actual no ha sido completada o ya terminamos
    ...
    // Eventos del teclado virtual en pantalla
