...
// ⭐ FUNCIÓN MODIFICADA PARA DIVISIONES EXACTAS ⭐
    function generateOperation() {
        // 1. Elegimos un divisor (bottomNum) entre 1 y 10 (el número de la tabla)
        const bottomNum = Math.floor(Math.random() * 10) + 1; // 1 a 10

        // 2. Elegimos el resultado/cociente (multiplier) también entre 1 y 10.
        const multiplier = Math.floor(Math.random() * 10) + 1; // 1 a 10

        // 3. El dividendo (topNum) es el producto, asegurando una división exacta.
        const topNum = bottomNum * multiplier;

        // Usamos el símbolo de división
        const operator = '÷';
        
        // 4. El resultado es el multiplicador original (cociente)
        const result = multiplier; 
        
        // topNum (Dividendo) es el número más grande (va arriba).
        // bottomNum (Divisor) es el número del 1 al 10 (va abajo).
