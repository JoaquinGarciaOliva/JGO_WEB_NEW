document.addEventListener('DOMContentLoaded', () => {
    ...
    // ✨ MODIFICACIÓN PRINCIPAL: Generar restas con Minuendo (0-20), Sustraendo (0-10) y Resultado >= 0.
        ...
        let numA = Math.floor(Math.random() * 21); // 0 a 20
        
        // 3. Asegurar que numA >= numB para garantizar resultado >= 0.
        while (numA < numB) {
            // Regenerar numA hasta que sea >= numB
            numA = Math.floor(Math.random() * 21); // Intentamos de nuevo de 0 a 20
        }
        
        const topNum = numA;
        const bottomNum = numB;
        
        const operator = '-'; 
        const result = topNum - bottomNum; // Resultado siempre >= 0 y <= 20
    ...
    // El listener se adjunta a *TODOS* los botones, incluyendo 'scoreMarker' y los nuevos.
